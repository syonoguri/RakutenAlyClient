...
(function(){
    window.onload = function(){
        ...
        /*------------------------------------------------------------------
        state:データの状態を保存する。内容が変更されるとvwメソッドを呼び出しブラウザ
            　に出力する。
        -------------------------------------------------------------------*/
        state.firstCaption = "検索キーワード";
        state.receivedResult = [];
        state.filteredResult = [];
        state.secondCaption = "";
        state.copyedResult = [];
        state.filteredCopyedResult = [];
        // エラーメッセージを格納
        state.makeShortResult = function(message){ 
            state.shortResult = message;
            vw.updateShortResult();
        };
        // 入力されたキーワードを格納
        state.makeCaption1 = function(caption){ 
            state.firstCaption = caption;
        };
        // 分析結果を格納
        state.makeResult1 = function(result){ 
            state.receivedResult = [];
            for(var i in result){
                state.receivedResult.push({score:result[i],keyword:i});
            }
            state.filteredResult = JSON.parse(JSON.stringify(state.receivedResult));
            vw.updateResult1(state.filteredResult);
            console.log(state.filteredResult);
            console.log(state.receivedResult);
        };
        // 分析結果を切り取り＆ペースト
        state.save = function(){ 
            state.copyedResult = state.receivedResult;
            state.secondCaption = state.firstCaption;
            state.receivedResult = [];
            state.filteredCopyedResult = JSON.parse(JSON.stringify(state.filteredResult));
            state.filteredResult = JSON.parse(JSON.stringify(state.receivedResult));
            state.firstCaption = "検索キーワード";
            vw.updateResult1(state.filteredResult);
            vw.updateResult2(state.filteredCopyedResult);
        };
        // 結果を反転
        state.ReverseFirstResult = function(){
            
            console.log(JSON.parse(JSON.stringify(state.filteredResult)));
            console.log(state.receivedResult);
            // this.fil1.reverse();
            state.filteredResult.reverse();
            console.log(JSON.parse(JSON.stringify(state.filteredResult)));
            console.log(state.receivedResult);

            vw.updateResult1(state.filteredResult);
        };
        state.ReverseCopyedResult = function(){
            state.filteredCopyedResult.reverse();
            vw.updateResult2(state.filteredCopyedResult);
        };
        state.filterResult = function(filter){
            state.filteredResult = [];
            console.log("keyword");
            let sentence = new RegExp(filter);
            for(let i of state.receivedResult){
                if(sentence.test(i["keyword"])){
                    console.log(i["keyword"]);
                    console.log(state.filteredResult);
                    state.filteredResult.push(i);
                }
            }
            vw.updateResult1(state.filteredResult);
        };
        state.filterCopyedResult = function(filter){
            state.filteredCopyedResult = [];
            let sentence = new RegExp(filter);
            for(let i of state.copyedResult){
                if(sentence.test(i["keyword"])){
                    state.filteredCopyedResult.push(i);
                }
            }
            vw.updateResult2(state.filteredCopyedResult);
        };
        state.ClearFirstFilter = function(){
            state.filterResult("");
        }
